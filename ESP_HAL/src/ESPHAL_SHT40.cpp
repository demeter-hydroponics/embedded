...
#define delay(x) _timeServer->delayUs(x * 1000U)
 ...
 */
Adafruit_SHT4x::Adafruit_SHT4x(TimeServer *timeServer, uint8_t i2cAddr) {
 ...
 */
bool Adafruit_SHT4x::begin(ESPHAL_I2C *i2cBus) {
    i2c_dev = i2cBus;
    if (temp_sensor) {
        delete temp_sensor;
    }
...
uint32_t Adafruit_SHT4x::readSerial(void) {
    ...
    if (i2c_dev->write(i2c_address, &cmd, 1) != HAL_I2C::ErrorCode::NO_ERROR) {
        return false;
    }
    delay(10);
    if (i2c_dev->read(i2c_address, reply, 6) != HAL_I2C::ErrorCode::NO_ERROR) {
        return false;
    }
...
*/
/**************************************************************************/
bool Adafruit_SHT4x::getEvent(sensors_event_t *humidity, sensors_event_t *temp) {
    uint32_t t = _timeServer->getUtimeUs() / 1000U;
    ...
    delay(duration);
    if (i2c_dev->read(i2c_address, readbuffer, 6) != HAL_I2C::ErrorCode::NO_ERROR) {
        return false;
    }

    if (readbuffer[2] != crc8(readbuffer, 2) || readbuffer[5] != crc8(readbuffer + 3, 2)) return false;
    ...
    _humidity = CONTROL_UTILS_CLAMP(_humidity, 0.0F, 100.0F);

    // use helpers to fill in the events
    if (temp) fillTempEvent(temp, t);
    if (humidity) fillHumidityEvent(humidity, t);
    return true;
}
...
void Adafruit_SHT4x::fillHumidityEvent(sensors_event_t *humidity, uint32_t timestamp) {
 ...
 */
Adafruit_Sensor *Adafruit_SHT4x::getHumiditySensor(void) { return humidity_sensor; }
 ...
 * @return Adafruit_Sensor*
 */
Adafruit_Sensor *Adafruit_SHT4x::getTemperatureSensor(void) { return temp_sensor; }
/**
...
bool Adafruit_SHT4x::writeCommand(uint16_t command) {
    ...
    return i2c_dev->write(i2c_address, cmd, 2) != HAL_I2C::ErrorCode::NO_ERROR;
}
 ...
 * @param cmd   The 16-bit command ID to send.
 */
bool Adafruit_SHT4x::readCommand(uint16_t command, uint8_t *buffer, uint8_t num_bytes) {
    uint8_t cmd[2];
    ...
    if (i2c_dev->write(i2c_address, cmd, 2) != HAL_I2C::ErrorCode::NO_ERROR) {
        return false;
    }
    if (i2c_dev->read(i2c_address, buffer, num_bytes) != HAL_I2C::ErrorCode::NO_ERROR) {
        return false;
    }
    return true;
}
...
BaseTempHumiditySense::ErrorCode Adafruit_SHT4x::getTemperature(float &temperature) {
    sensors_event_t event;
    if (getEvent(&event, NULL)) {
        temperature = event.temperature;
        return BaseTempHumiditySense::ErrorCode::NO_ERROR;
    }
    return BaseTempHumiditySense::ErrorCode::DEVICE_ERROR;
}

BaseTempHumiditySense::ErrorCode Adafruit_SHT4x::getHumidity(float &humidity) {
    sensors_event_t event;
    if (getEvent(NULL, &event)) {
