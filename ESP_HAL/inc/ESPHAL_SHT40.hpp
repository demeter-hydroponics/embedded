 ...
 */
class Adafruit_SHT4x : public BaseTempHumiditySense {
   public:
    Adafruit_SHT4x(TimeServer *timeServer, uint8_t i2cAddr = SHT4x_DEFAULT_ADDR);
    ~Adafruit_SHT4x(void);

    bool begin(ESPHAL_I2C *i2cBus);
    ...
    ErrorCode getTemperature(float &temperature) override;
    ErrorCode getHumidity(float &humidity) override;
   ...
   protected:
    ...
    ESPHAL_I2C *i2c_dev = NULL;                       ///< Pointer to I2C bus interface
    uint8_t i2c_address = SHT4x_DEFAULT_ADDR;         ///< I2C address
